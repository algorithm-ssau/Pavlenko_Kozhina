...
from main_dir.generator import BatchGenerator
from main_dir.data.grammeme_vectorizer import GrammemeVectorizer
from main_dir.data.word_vocabulary import WordVocabulary
from main_dir.data.loader import Loader
from main_dir.char_model import build_dense_chars_layer, get_char_model
...
class LSTMAnalysis:
    ...
    def load_train(self, config: ModelBuild, model_config_path: str = None, model_weights_path: str = None):
    ...
    def load_eval(self, config: ModelBuild, eval_model_config_path: str,
    ...
    def build(self, config: ModelBuild, word_embeddings=None):
        inputs = []
        embeddings = []
    ...
    def train(self, file_names: List[str], train_config: TrainBuild, build_config: ModelBuild) -> None:
        file = open("main_dir/test/agree.txt", "w")
        file.close()
        file = open("main_dir/test/failed.txt", "w")
        file.close()
        train_acc = []
        train_loss = []
    ...
    def evaluate(self, file_names, val_idx, train_config: TrainBuild, build_config: ModelBuild) -> None:
        word_count = 0
        word_errors = 0
        sentence_count = 0
        sentence_errors = 0
    ...
    def predict_probabilities(self, sentences: List[List[str]], batch_size: int, build_config: ModelBuild) -> List[
        List[List[float]]]:
